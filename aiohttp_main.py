...
async def get_category_urls(homepage, session):
...
async def get_book_urls(category_url, session, semaphore):
...
async def get_book_details(book_url, session, semaphore):
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        try:
            ...
            cat_urls = await get_category_urls(homepage, session)
            
            # 2. Get all book URLs from all categories
            nested_book_urls = await asyncio.gather(*[get_book_urls(url, session, semaphore) for url in cat_urls])
            ...
            data = await asyncio.gather(*[get_book_details(url, session, semaphore) for url in book_urls])
